...
static int wd_aead_sess_eops_init(struct wd_aead_sess *sess)
{
	...
	for (i = 0; i < config->ctx_num; i++) {
		...
		if (!ret && config->ctxs[i].extend_ops) {
			...
			ret = sess->eops.eops_aiv_init(drv, &sess->mm_ops,
						       &sess->eops.params);
			if (ret) {
			...
			memcpy(&sess->eops, eops, sizeof(*eops));
			sess->eops.params = drv;
			break;
		}
	}
...
static void wd_aead_sess_eops_uninit(struct wd_aead_sess *sess)
{
	...
	if (sess->eops.eops_aiv_uninit) {
		drv = sess->eops.params;
		sess->eops.eops_aiv_uninit(drv, &sess->mm_ops,
					   sess->eops.params);
...
static int wd_aead_init_nolock(struct wd_ctx_config *config, struct wd_sched *sched, void *attrs)
{
	...
	wd_aead_setting.priv = STATUS_ENABLE;
	return 0;
...
int wd_aead_init(struct wd_ctx_config *config, struct wd_sched *sched)
{
	...
	ret = wd_ctx_drv_config("gcm(aes)", &wd_aead_setting.config);
	if (ret)
		   ...
		   struct wd_ctx_params *ctx_params)
{
	...
	ret = wd_ctx_drv_config(alg, &wd_aead_setting.config);
	if (ret)
		goto out_uninit_nolock;
